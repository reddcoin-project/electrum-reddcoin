...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-Redd.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-redd-macos
	cd /tmp/electrum-redd-macos
...
rm -rf /tmp/electrum-redd-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-redd-macos/image/
cp -r $1 /tmp/electrum-redd-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-Redd_uncompressed.dmg \
    /tmp/electrum-redd-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-Redd_uncompressed.dmg electrum-redd-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_uncompressed.dmg

echo "Done."
sha256sum electrum-redd-$VERSION.dmg
