...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Redd"
  !define PRODUCT_WEB_SITE "https://github.com/reddcoin-project/electrum-redd"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-redd-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-redd\electrum_redd\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-redd\*.*"
  File "c:\electrum-redd\electrum_redd\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-redd-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-redd-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-redd-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-redd-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-redd-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\reddcoin\shell\open\command" "" "$\"$INSTDIR\electrum-redd-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\reddcoin"
