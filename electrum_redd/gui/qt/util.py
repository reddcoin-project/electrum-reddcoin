...
from electrum_redd.i18n import _, languages
from electrum_redd.util import FileImportFailed, FileExportFailed, make_aiohttp_session, resource_path
from electrum_redd.invoices import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING
...
def export_meta_gui(electrum_window, title, exporter):
    ...
    filename = electrum_window.getSaveFileName(_("Select file to save your {}").format(title),
                                               'electrum-redd_{}.json'.format(title), filter_)
    if not filename:
        return
    try:
        exporter(filename)
