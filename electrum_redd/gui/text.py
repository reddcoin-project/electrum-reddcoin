...
from electrum_redd.util import format_satoshis
from electrum_redd.bitcoin import is_address, COIN
from electrum_redd.transaction import PartialTxOutput
from electrum_redd.wallet import Wallet
from electrum_redd.wallet_db import WalletDB
from electrum_redd.storage import WalletStorage
from electrum_redd.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from electrum_redd.interface import ServerAddr
from electrum_redd.logging import console_stderr_handler

if TYPE_CHECKING:
    from electrum_redd.daemon import Daemon
...
class ElectrumGui:
    ...
    def __init__(self, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrum-redd create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = electrum_redd.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
