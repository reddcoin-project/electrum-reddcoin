...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_redd."):
...
electrum_logger = logging.getLogger("electrum_redd")
electrum_logger.setLevel(logging.DEBUG)


def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_redd_log_*.log")), reverse=True)
    for f in files[keep:]:
        os.remove(str(f))
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_redd_log_{timestamp}_{PID}.log"
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_redd."):
        name = name[9:]
...
def configure_logging(config):
    ...
    if is_android:
        ...
        build_config = autoclass("org.electrum_redd.electrum_redd.BuildConfig")
    ...
    _logger.info(f"Electrum-Redd version: {ELECTRUM_VERSION} - https://reddcoin.com - {GIT_REPO_URL}")
